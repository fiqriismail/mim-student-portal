...
| AD-2 | In-process mediator library vs hand-rolled | Open | MediatR (license terms changed in recent versions — confirm current licensing before adopting) vs a small custom `ICommandHandler<TCommand,TResult>` interface |
| AD-3 | Background job runner for email retries | Open, defaults to a hosted `Channel<T>` service | Escalate to Hangfire/Quartz if retry/observability needs grow beyond AC-5.2.4's "at least 3 retries with backoff" |
| AD-4 | Identity: ASP.NET Identity vs custom slim layer | Open, leaning custom | PRD's `User`/`Token` shape is narrower than ASP.NET Identity's defaults; a thin custom layer avoids fighting Identity's schema assumptions |
